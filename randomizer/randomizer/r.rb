#ランダマイザ
#いるもの ruby 3.0 [x64-mingw32], ImageMagick 7.0.11-8 Q16 x64, RMagick 4.2.2
#実行 ruby r.rb [ファイル名] [x軸の分割数]
#特殊なオプション [ファイル名] => :a(すべてのファイル), :l(まだ実行されたことがないファイル)
#特殊なオプション [x軸の分割数] => :r(ランダム, 1ファイルごとに変わる)
...
if ARGV[1].nil?
    sep_x = 5 #指定しなければすべて5
elsif ARGV[1] == ":r" #:rならばスルー,1ファイルごとに変化するのでそのときランダムに決める
else
...
for idx_file in 0...files.size #対象のファイルの数だけ繰り返す
    ...
    rotate = sep.times.map{|i| (i == 0) ? 0 : rand(4) } #断片画像の回転を決める, 出力画像のi番目の断片は90*rotate[i]度回転している
    ...
    for i in 0...sep
        tiles[random[i]].rotate!(90 * (rotate[i])) if i != 0 #回転, 左上の断片は回転させないとあったのでその通りにしている
    ...
    #位置と回転の情報をマーシャルデータにしてファイルに出力
    # File.open("./output/MarshalData/#{filename}", "w") do |f|
    #     Marshal.dump(random, f)
    #     Marshal.dump(rotate, f)
    # end
end
